...
try {
  for (const entry of PLATFORM_COVERAGE) {
    ...
    try {
      ...
      const detected = getCoveragePlatform(finalUrl) ?? getCoveragePlatform(entry.liveCheckUrl);
      const reachable = Boolean(response && response.status() < 400);
      const strictFieldsPresent = fields.title && fields.company && fields.location && fields.description;
      const passed = reachable && detected === entry.platform && (entry.capture !== "verified" || strictFieldsPresent);
      result = { platform: entry.platform, sourceUrl: entry.liveCheckUrl, finalUrl, statusCode: response?.status() ?? null, detected, fields, passed, failureReason: passed ? null : !reachable ? "page_unreachable" : detected !== entry.platform ? "platform_not_detected" : "required_fields_missing" };
    } catch (error) {
      result = { platform: entry.platform, sourceUrl: entry.liveCheckUrl, finalUrl: page.url(), statusCode: null, detected: null, fields: { title:false, company:false, location:false, description:false }, passed:false, failureReason: error instanceof Error ? error.message.slice(0, 300) : "live_check_failed" };
...
const rows = results.map((item) => `<tr><td>${item.platform}</td><td>${item.passed ? "PASS" : "FAIL"}</td><td>${item.statusCode ?? "—"}</td><td>${item.detected ?? "—"}</td><td>${Object.entries(item.fields).filter(([,value])=>value).map(([key])=>key).join(", ") || "none"}</td><td>${item.failureReason ?? ""}</td></tr>`).join("");
