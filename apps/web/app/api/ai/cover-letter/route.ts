...
const cvSchema = z.object({ contact:z.object({name:z.string().max(200),email:z.string().max(200),phone:z.string().max(50),location:z.string().max(200),linkedin:z.string().max(300).optional()}),summary:z.string().max(4000),experience:z.array(z.object({title:z.string().max(200),company:z.string().max(200),dates:z.string().max(100),bullets:z.array(z.string().max(1000)).max(40)})).max(40),education:z.array(z.object({degree:z.string().max(200),institution:z.string().max(200),dates:z.string().max(100)})).max(20),skills:z.array(z.string().max(100)).max(200) });
...
export async function POST(request:NextRequest){
  ...
  }catch(error){const status=error instanceof z.ZodError?400:error instanceof FeatureGateError?402:error instanceof RateLimitError?429:500;return NextResponse.json({data:null,error:toPublicApiError(error instanceof Error?error.message:"Cover-letter generation failed",status)},{status});}
}

export async function PATCH(request:NextRequest){try{const{user}=await getRequestUser(request);if(!user)return NextResponse.json({data:null,error:"Unauthorised"},{status:401});const body=updateSchema.parse(await request.json());const{data,error}=await createAdminClient().from("cover_letters").update({content:body.content}).eq("id",body.id).eq("user_id",user.id).select("id,version,content").single();if(error)throw error;return NextResponse.json({data:{letter:data},error:null});}catch(error){const status=error instanceof z.ZodError?400:500;return NextResponse.json({data:null,error:toPublicApiError(error instanceof Error?error.message:"Cover-letter update failed",status)},{status});}}
