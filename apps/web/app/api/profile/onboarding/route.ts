...
const humanName = z
  ...
  .min(2)
  .max(100)
  .regex(/^[\p{L}\p{M}][\p{L}\p{M}' .-]+$/u, "Invalid name");
const placeName = z
  .string()
  .trim()
  .min(2)
  .max(120)
  .regex(/^[\p{L}\p{M}][\p{L}\p{M}' .,-]+$/u, "Invalid location or country");
...
const patchSchema = z
  .object({
    ...
    phone: z
      ...
      .or(z.literal(""))
      .optional(),
    countryCurrent: placeName.optional(),
    countriesTarget: z.array(placeName).min(1).max(20).optional(),
    linkedinUrl: optionalWebUrl.optional(),
    githubUrl: optionalWebUrl.optional(),
    portfolioUrl: optionalWebUrl.optional(),
    workAuthorisationCategory: z
      .enum([
      ...
      .optional(),
    workRightDetails: z.string().trim().max(2000).optional(),
    baseCvText: z.string().max(100_000).optional(),
    onboardingStep: z.number().int().min(0).max(6).optional(),
    complete: z.boolean().optional(),
  })
  .superRefine((body, ctx) => {
    if (
      ((body.onboardingStep ?? 0) >= 3 || body.complete) &&
      !isValidLinkedInProfile(body.linkedinUrl)
    )
      ctx.addIssue({
        code: "custom",
        path: ["linkedinUrl"],
    ...
    if (body.complete) {
      const missing = getMissingOnboardingEvidence({
      ...
      for (const field of missing)
        ctx.addIssue({
          code: "custom",
          path: [field],
          message: `Complete ${field} before finishing onboarding`,
        });
    }
  });

const select =
  "full_name,email,phone,photo_url,country_current,countries_target,current_country,target_countries,linkedin_url,github_url,portfolio_url,target_roles,work_authorisation_category,work_right_details,base_cv_text,experience_highlights,project_summaries,onboarding_step,onboarding_completed_at,alert_frequency,alert_last_sent_at";
export async function GET(request: NextRequest) {
  try {
    ...
    const countries = data?.countries_target?.length
      ? data.countries_target
    ...
    const normalised = data
      ? {
          ...data,
          country_current: data.country_current || data.current_country,
    ...
    return NextResponse.json(
      {
        data: normalised
          ? {
              ...normalised,
              onboarding_ready: hasCompletedRequiredOnboarding(normalised),
              onboarding_missing: getMissingOnboardingEvidence(normalised),
            }
          : null,
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const countries = body.countriesTarget;
    const changes = {
      ...(body.fullName !== undefined && { full_name: body.fullName }),
      ...(body.email !== undefined && { email: body.email || null }),
      ...(body.phone !== undefined && { phone: body.phone || null }),
      ...(body.countryCurrent !== undefined && {
        country_current: body.countryCurrent,
        current_country: body.countryCurrent,
      }),
      ...(countries !== undefined && {
        countries_target: countries,
      ...
      ...(body.workAuthorisationCategory !== undefined && {
        work_authorisation_category: body.workAuthorisationCategory,
        sponsorship_needed:
          body.workAuthorisationCategory === "sponsorship_required",
      }),
      ...(body.workRightDetails !== undefined && {
        work_right_details: body.workRightDetails,
      }),
      ...(body.baseCvText !== undefined && { base_cv_text: body.baseCvText }),
      ...(body.onboardingStep !== undefined && {
        onboarding_step: body.onboardingStep,
      }),
      ...(body.complete
        ? {
