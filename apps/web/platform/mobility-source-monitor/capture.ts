...
export async function captureOfficialSourceArtifact(
  ...
  try {
    ...
    if (Buffer.byteLength(content, "utf8") > 5_242_880) return { observation: { available: true, httpStatus: response.status, rawSha256: hash(content), normalizedSha256: null, parserVersion: SOURCE_PARSER_VERSION, normalizerVersion: SOURCE_NORMALIZER_VERSION }, content: null, contentType: "text/plain", language: "und" }
