...
export default function OutreachWorkspace() {
  ...
  const loadContacts = async () => { const response = await fetch("/api/outreach/contacts"); const payload = await response.json(); if (response.ok) setContacts(payload.data ?? []); else setStatus(payload.error); };
  useEffect(() => { void load(); void loadContacts(); }, []);
