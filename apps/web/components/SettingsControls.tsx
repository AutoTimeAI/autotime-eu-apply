...
export function SettingsControls({
  ...
  const openBillingPortal = async () => {
    try {
      ...
      setStatus(null)
      const route = plan === "pro" ? "/api/stripe/portal" : "/api/stripe/checkout"
  ...
  const deleteAccountProfile = async () => {
    if (
      !window.confirm(
        "Delete the profile saved to this account? Your browser backup stays on this device."
      )
    ) {
      return
    }
    ...
    try {
      ...
      setStatus(null)
      const response = await fetch("/api/sync/profile", { method: "DELETE" })
      const payload = (await response.json()) as { error: string | null }

      if (!response.ok || payload.error) {
        setStatus(payload.error ?? "Profile could not be deleted.")
        return
      }

      setStatus("Account profile deleted. Browser backup remains available.")
    } catch (error: unknown) {
      setStatus(
    ...
    <section className="settings-control-grid" aria-label="Settings controls">
      ...
      <article className="settings-control-panel" id="billing-controls">
        <div className="section-heading">
          ...
          <p>Open checkout or the billing portal without duplicating pricing.</p>
        </div>
        <button
      ...
      <article className="settings-control-panel" id="data-controls">
        <div className="section-heading">
          ...
          <p>Export the browser backup or delete the profile saved online.</p>
        </div>
        <div className="profile-action-row">
          <button
      ...
      <article className="settings-control-panel">
        <div className="section-heading">
          ...
          <h2>Defaults</h2>
          <p>Keep lightweight defaults here; evidence stays in Profile Evidence.</p>
        </div>
        <label>
          Default region
          <input
