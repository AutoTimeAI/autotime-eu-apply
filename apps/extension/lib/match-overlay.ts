...
// Scoring never fetches directly from this content script - it asks the
// background worker to do it, so the raw authToken never enters this
// script's execution context (which runs injected into every visited page).
async function requestJobMatchScore(payload:{url?:string;title?:string;description?:string}):Promise<Score|null>{
  try{
    const response=await chrome.runtime.sendMessage({type:"AUTOTIME_SCORE_JOB",payload}) as {ok?:boolean;data?:Score}|undefined;
...
export async function showEscoMatchOverlay(){
  ...
  const state=await getConnectionState();if(!state.connected)return;
  ...
  if(!payload)return;
  const score=await requestJobMatchScore(payload);if(!score?.matched||!score.totalEssentialSkills)return;
  const host=document.createElement("aside");host.id="autotime-esco-match-host";host.setAttribute("aria-label","AutoTime ESCO job match");const root=host.attachShadow({mode:"open"});
...
async function scoreCurrentLinkedInPage(){
  ...
  const score=await requestJobMatchScore({title:details.roleTitle,description:details.jobDescription});
  if(!score?.matched||!score.totalEssentialSkills)return;renderScore(score,"LinkedIn · explicit check");
}
