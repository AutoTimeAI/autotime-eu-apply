...
| Production DB performance advisor scan | **Complete - the two WARN findings fixed** | 230 findings via subagent review. The two WARN-level items - 119 `auth_rls_initplan` (per-row `auth.uid()` re-evaluation) across `profiles`/`applications`/`evidence_records`/etc, and 4 duplicate-policy findings on `custom_job_sources` - were fixed in `20260919190000_rls_initplan_and_duplicate_policy_hardening.sql`, applied to production via the Supabase SQL editor and re-verified live: `pg_policies` now shows `(( SELECT auth.uid() AS uid) = user_id)` on `profiles`/etc, and the 3 redundant `custom_job_sources_*_own` policies are gone, leaving only the pre-existing `ALL` policy and the untouched `_insert_own` one. Rest (76 unindexed-FK + 31 unused-index) is INFO-level and safe to defer to a post-launch pass driven by real query-log data | Resolved |
