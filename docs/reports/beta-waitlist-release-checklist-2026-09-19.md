...
| `BETA_INVITE_CODE` env var set | **Pending - blocks self-serve unlock** | Attempted via the Vercel integration; blocked by a real Vercel API 403 ("Additional permissions are required to create production environment variables") - this integration's token cannot write production env vars. **Manual action needed**: add `BETA_INVITE_CODE` in Vercel → Project Settings → Environment Variables (Production). Suggested value generated this session: `OIXUDVYG` (or choose your own). Until set, `getBetaInviteCode()` returns `null` and every redeem attempt is correctly rejected rather than silently accepted - fails closed, not open, but nobody can self-unlock until this is set | Blocks self-serve invite-code unlock only - admin manual approval (existing `beta_access` admin route) still works without this |
...
What's actually missing before calling this beta release fully verified is
short and concrete, not open-ended:

1. Set `BETA_INVITE_CODE` in Vercel (manual step, blocked by an integration
   permission limit - see the row above).
2. One real human pass signing in with a genuinely new account to confirm
   the waitlist page itself renders and reads correctly.
3. One real human pass redeeming a real invite code end-to-end.
4. One real human pass confirming the admin manual-approval fallback still
   works for a truly pending (not suspended) user.

None of these require further engineering - they are verification steps
ready to execute once the env var is set.
