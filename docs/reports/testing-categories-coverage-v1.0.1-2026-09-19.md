...
**Update: rollback readiness moved from Partial to Pass** (real live rehearsal run, named operator confirmed). **Result: 5 of 8 P0 items are a clean Pass. 3 remain Partial**: authorisation/cross-user isolation (structural evidence only - blocked by a missing local credential for a real two-account test, not skipped by choice), database recovery (now a **confirmed Fail**, not just unverified - the Supabase project has zero backup coverage), and security-and-privacy (security passes; privacy is founder-owned and open). Per the stated rule, **any P0 that is Partial/Open/Fail counts as NO GO for an unqualified release** - consistent with the dossier's existing decision.
