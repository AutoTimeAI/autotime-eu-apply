...
| No functional test coverage of the actual webhook/checkout handler logic - existing tests only verify config wiring via source-text regex, never invoke `handleStripeEvent`/`mapStripeStatus`/`upsertSubscriptionFromStripe`/`grantCreditPack` with mock Stripe events | MEDIUM | **Partially fixed** - extracted the pure, DB-free logic (status mapping, period-end selection, customer-id extraction, all 5 object type guards, and a new `validateCreditPackMetadata` pulled out of `grantCreditPack`'s inline validation) into `lib/stripe-webhook-logic.ts`, byte-for-byte behavior preserved, and added `apps/web/tests/stripe-webhook-logic.test.mjs`: 19 tests against real Stripe-shaped fixtures, now wired into `test:unit`. The DB-touching async handlers (`upsertSubscriptionFromStripe`, `claimStripeEvent`, the webhook `POST` handler itself) remain untested - refactoring live billing code for full dependency-injectable testability is a larger, separate change with more regression surface than was proportionate to do in the same pass |
