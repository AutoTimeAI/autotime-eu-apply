...
- Automated release check: `docs\release-runs\2026-05-06T14-05-42-344Z.md`
- MVP automation report: `docs\automation-runs\2026-05-06T14-06-15-619Z.md`
- Python MVP smoke report: `docs\automation-runs\python-smoke-20260506-150528\report.md`

## Automated Testing Gate

Run these before manual release validation and link the generated reports above.

- [x] `pnpm release:check` passed
- [x] `SKIP_LIVE_SMOKE=1 pnpm test:mvp` passed for offline/local validation, or `pnpm test:mvp` passed with deployed smoke enabled
- [x] `pnpm build:extension` produced `apps/extension/.output/chrome-mv3`
- [x] `pnpm build:web` passed if V2 dashboard changed
- [x] `pnpm test:smoke:web` passed if dashboard smoke markers changed
- [x] `pnpm test:web:interview` passed if interview prep changed
