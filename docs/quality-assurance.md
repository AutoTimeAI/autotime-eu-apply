...
| #177 | **The main CV-generation gate had zero Stamp4/international awareness, and the function documented as its intended replacement was never wired up anywhere**: found by a functional audit tracing what actually enforces content generation, independently re-verified with two parallel research agents before any code changed. `apps/web/app/api/ai/content/route.ts`'s `getContentGuardrailIssues` - the function whose output directly gates a 422 block on `/api/ai/content` - only ever called the deprecated `evaluateCountryFit` (`fit-model.ts`, marked deprecated in its own file header pointing at `international/orchestration.ts`'s `orchestrateJobDecision` as "the current replacement"). Confirmed `orchestrateJobDecision` - which documents itself as "the only boundary allowed to issue a final cross-domain job decision" - had zero callers anywhere in `apps/web`: the deprecation notice pointed at a replacement that was never actually connected. Concrete consequence: a job could clear the main dashboard's CV-generation gate while the same job/country got a genuinely different, Stamp4-verified blocker on the standalone International Module page (`PR #193`, merged the same day) - nothing reconciled the two views, since the deprecated engine has no knowledge Stamp4 integration exists at all. Fixed by composing `evaluateAutoTimeFitScore` (role/skill fit - not deprecated, already live elsewhere unchanged) with `assessInternationalJob` (fed by the already-existing `migrateCandidateProfileToMobilityProfile` bridge, plus a live Stamp4 statutory-threshold check for the 4 countries it covers) through `orchestrateJobDecision`, and blocking on its `"Skip"`/`"Insufficient evidence"` decisions - the direct translation of the old gate's "confirmed blocker" semantics onto the new engine, not a new policy. New behavioral tests for `orchestrateJobDecision` (previously untested despite being the documented final decision boundary) plus a static-inspection test confirming the route actually calls the new composition and no longer references the deprecated function, both confirmed to fail against the old wiring before verifying the fix | `apps/web/app/api/ai/content/route.ts` |
