-- Preserve privacy-safe redirect provenance for every newly captured source version.
begin;

create or replace function public.record_and_propagate_mobility_source_change(
...
begin
  ...
  if p_observed_version is not null then
    source_redirect_chain := coalesce(p_observed_version->'redirectChain', '[]'::jsonb);
    if jsonb_typeof(source_redirect_chain) <> 'array' then
      raise exception using errcode = '22023', message = 'invalid source redirect chain';
    end if;
    ...
    insert into public.mobility_source_versions (
      ...
      p_observed_version->>'snapshotUri', source_redirect_chain,
